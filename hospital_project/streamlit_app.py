...
menu = st.sidebar.selectbox("Navigation", [
    "Add Patient", "Add Doctor", "Create Appointment", "Cancel Appointment", 
...
if menu == "Add Patient":
    ...
    if not system.patients or not system.doctors:
        st.warning("Add at least one patient and doctor first.")
    else:
    ...
    if not system.patients or not system.doctors:
        st.warning("Cancel at least one patient and doctor first.")
    else:
        with st.form("appointment_form"):
            aid = st.text_input("Appointment ID")
            date = st.date_input("Date")
            ...
            if submitted:
                if aid and patient and doctor and date:
                    appt = Appointment(aid, patient, doctor, date)
                    system.cancel_appointment(appt)
                    st.success(f"Appointment canceled between {patient.name} and Dr. {doctor.name}.")
                else:
                    st.error("Please fill in all fields.")
    ...
    if system.patients:
        ...
        st.download_button("Download Patients CSV", patient_csv, "patients.csv", "text/csv")
    ...
    if system.doctors:
        ...
        st.download_button("Download Doctors CSV", doctor_csv, "doctors.csv", "text/csv")
    ...
    if system.appointments:
        ...
        st.download_button("Download Appointments CSV", appt_csv, "appointments.csv", "text/csv")
